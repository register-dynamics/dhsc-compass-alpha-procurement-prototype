...
export async function down(db: Kysely<any>): Promise<void> {
  await db.withSchema('app').schema
    .dropView('make_documents')
    .execute()

  await db.schema
    .dropTable('app.evidence')
    .execute()
...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.insertInto('app.evidence')
    .columns(['evidence_id','created_at', 'assessment_date', 'assessment_date_desc', 'rating', 'rating_type', 'organisation_id', 'procured','scale','ward_department','summary'])
    .expression((eb) => eb
  ...
  await db.schema
    .alterTable('app.documents')
    .addColumn('evidence_id', 'integer', (col) => col.references('app.evidence.evidence_id'))
  ...
  await db.updateTable('app.documents')
    .set((eb) => ({ evidence_id: sql<string>`document_id` }))
    .execute()

  await db.schema
    .alterTable('app.documents')
    .alterColumn('evidence_id', (col) =>
      col.setNotNull())
    .execute()
  ...
  await db.schema
    ...
    .addColumn('evidence_id', 'integer', (col) =>
      col.references('app.evidence.evidence_id'), // .notNull()
    )
    .execute()

  await db.updateTable('app.product_matches')
    .set((eb) => ({ evidence_id: sql<string>`document_id` }))
    .execute()
